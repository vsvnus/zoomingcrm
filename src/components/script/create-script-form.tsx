...
export function CreateScriptForm({ projectId, projectTitle }: CreateScriptFormProps) {
  const router = useRouter()
  const [isCreating, setIsCreating] = useState(false)
  const [title, setTitle] = useState(`Roteiro - ${projectTitle}`)
      ...
      >
        ...
        <div className="text-center space-y-3">
          ...
          <p className="text-text-secondary">
            Configure os detalhes do roteiro para <span className="font-medium text-text-primary">{projectTitle}</span>
          </p>
        </div>
