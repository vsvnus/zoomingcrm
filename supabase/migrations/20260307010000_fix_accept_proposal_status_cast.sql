-- ==============================================================================
-- FIX: accept_proposal_v1 - cast explícito de status para enum payment_status
-- CAUSA RAIZ: a expressão CASE retorna tipo `text`, mas financial_transactions.status
-- é do tipo enum `payment_status`, causando erro de tipo na rota de payment_schedule.
-- ==============================================================================
...
BEGIN
  ...
  v_is_recurring := COALESCE(v_proposal.is_recurring, false);
  ...
  SELECT
    ...
    'Gravacao referente ao projeto: ' || v_proposal.title,
    recording_date,
    recording_date + interval '4 hours',
    v_project_id,
    v_proposal.organization_id,
    'shooting',
    p_user_id
  ...
  -- Check for custom payment schedule first
  ...
  IF v_schedule_count > 0 THEN
    -- USE CUSTOM SCHEDULE
    -- FIX: Cast explícito para enum payment_status (antes retornava text, causando erro)
    ...
    SELECT
      ...
      'INCOME',
      'CLIENT_PAYMENT',
      'Pagamento Proposta: ' || ps.description,
      ps.amount,
      (CASE WHEN ps.paid = true THEN 'PAID' ELSE 'PENDING' END)::payment_status,
      v_project_id,
      v_proposal.id,
      v_proposal.client_id,
      ps.due_date,
      NOW(),
      v_is_recurring,
      CASE WHEN v_is_recurring THEN 'MONTHLY' ELSE NULL END
    FROM payment_schedule ps
    WHERE ps.proposal_id = p_proposal_id
    ORDER BY ps."order" ASC;

  ELSE
    -- FALLBACK TO LEGACY LOGIC (Installments division)
